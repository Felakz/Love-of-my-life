...
const Hero = () => {
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-black">
      ...
      {/* Enhanced Scroll Indicator with tech effects */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <motion.div
          animate={{ y: [0, 10, 0] }}
          transition={{ duration: 2, repeat: Infinity }}
          className="relative w-6 h-10 border-2 border-white/30 rounded-full flex justify-center hover:border-gold-400/50 transition-colors cursor-pointer"
          whileHover={{ scale: 1.1 }}
        >
          {/* Scanning effect inside scroll indicator */}
          <motion.div
            animate={{ 
              y: [0, 20, 0],
              opacity: [0, 1, 0] 
            ...
            transition={{ duration: 2, repeat: Infinity }}
            className="absolute top-1 w-1 h-1 bg-gold-400 rounded-full"
          />
          
          <motion.div 
            className="w-1 h-3 bg-gradient-to-b from-white/60 to-gold-400/60 rounded-full mt-2"
            animate={{ 
          ...
          {/* Energy rings around scroll indicator */}
          {[...Array(2)].map((_, i) => (
            <motion.div
              key={i}
              animate={{
              ...
              className="absolute inset-0 border border-gold-400/30 rounded-full"
            />
          ))}
        </motion.div>
        ...
        <motion.p
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 2 }}
          className="text-white/50 text-xs mt-2 text-center relative"
        >
          <motion.span
            animate={{
              textShadow: [
          ...
          {[...Array(4)].map((_, i) => (
            <motion.div
              ...
              className="absolute w-0.5 h-0.5 bg-gold-400 rounded-full"
              style={{
                left: '50%',
                top: '50%',
              }}
